 ...
 * 
 * Attention: this code does not implement Daylight Savings Time, so the user is require to adjust the timezone value in order to compensate for this
*/
...
//const char* ntpServerName = "time.nist.gov";  //"time4.google.com"; //     /*use this if the timeserver needs to be hardcoded, in that case NTP_init isn't required and can be commented*/
char ntpServerName[32];
int tzone = 0;          /*use value = 3600 (1*60*60) for UTC+1, use value -3600 (-1*60*60) for UTC-1 etc.
...
void NTP_init(String ntpserv, int timezone)
{  
  ...
  {   
    ...
    case NTP_WAITFORPACKET:
    {
      ...
      {      
        ...
        NTP_struct.epoch = NTP_struct.epoch + (unsigned long)tzone;       /*correct for timezone*/
      ...
      else
      {
        ...
        {                                                     /*and we must do another request*/
          ...
          if(retry_count > 0)
          {
             Serial.println("request new packet");              
...
{
  Serial.print("Local time ");           /*show the time*/    
